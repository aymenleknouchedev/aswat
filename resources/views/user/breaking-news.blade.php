...
@section('content')
    ...
    <div class="web">
        ...
        <div class="container">
            <div class="title">
                <p class="section-title">أخبار عاجلة</p>
            ...
            <div class="newCategory-all-section">
                ...
                <div class="newCategory-all-list">
                    @if (isset($breakingContents) && count($breakingContents) > 0)
                        <div id="breaking-content-container">
                            @include('user.partials.breaking-news-items', ['breakingContents' => $breakingContents])
                        </div>

                        @if (isset($totalBreaking) && $totalBreaking > count($breakingContents))
                            <div class="text-center mt-3" id="breaking-load-more-container">
        ...
        <div class="mobile-flow">
            <div class="mobile-container" style="margin-top: 68px;">
                <div class="mobile-simple-list" dir="rtl">
                    <h2 class="mobile-simple-header">أخبار عاجلة</h2>
                    <div style="padding: 0px 16px">
                        @include('user.components.ligne')
                    </div>
                    <ul class="mobile-simple-ul" role="list" id="mobile-breaking-container">
                        @forelse ($breakingContents as $item)
                            <li class="mobile-simple-item">
                                <a href="{{ route('news.show', ['news' => $item->id]) }}" class="mobile-more-link">
                                    <div class="ms-text">
                                        <p class="ms-title">{{ $item->text }}</p>
                                    </div>
                                </a>
                            </li>
                        @empty
                            <li style="padding: 12px 0; text-align: center; color: #999;">لا توجد أخبار عاجلة حالياً.</li>
                        @endforelse
                    </ul>
                </div>
            </div>
...
<script>
    // Breaking news load more (desktop)
    let breakingLoading = false;

    document.addEventListener("click", async function(e) {
        const btn = e.target.closest('#breaking-load-more-container .photos-load-more-btn');
        if (!btn) return;
        if (breakingLoading) return;
        ...
        try {
            let response = await fetch(`{{ route('breakingNews') }}?page=${page}`, {
                headers: {
            ...
            if (data.trim().length === 0) {
                btn.closest("#breaking-load-more-container")?.remove();
            } else {
                let container = document.querySelector("#breaking-content-container");
                if (container) {
            ...
            console.error("Error loading more breaking news:", error);

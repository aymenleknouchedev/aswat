{{-- ====================== SOCIAL MEDIA (Styles) ====================== --}}
<style>
    :root { --az-border:#e5e7eb; --az-muted:#6b7280; }
    .image-preview-container img{ max-height:200px; }

    /* nbn Picker */
    #nbnMediaPicker, #nbnMediaPicker *{ box-sizing:border-box; }
    .nbn-modal{ position:fixed; inset:0; display:none; z-index:2075; }
    .nbn-modal[aria-hidden="false"]{ display:flex; }
    .nbn-backdrop{ position:absolute; inset:0; background:rgba(0,0,0,.5); z-index:2070; }
    .nbn-box{ position:relative; margin:auto; width:min(960px,92vw); max-height:90vh; background:#fff; color:#111;
        display:flex; flex-direction:column; overflow:hidden; border-radius:.5rem; box-shadow:0 10px 30px rgba(0,0,0,.15); z-index:2076; }
    .nbn-head{ display:flex; align-items:center; justify-content:space-between; padding:12px 16px; border-bottom:1px solid var(--az-border); }
    .nbn-x{ background:transparent; border:0; font-size:1.6rem; cursor:pointer; color:#666; }
    .nbn-tabs{ display:flex; padding:8px 16px; border-bottom:1px solid var(--az-border); }
    .nbn-tab{ border:1px solid var(--az-border); background:#fff; padding:.45rem .8rem; cursor:pointer; font-weight:600; margin-right:8px; border-radius:.35rem; }
    .nbn-tab.is-active{ border-color:#cfcfcf; }
    .nbn-panel[hidden]{ display:none; }
    .nbn-filters{ display:flex; padding:12px 16px; border-bottom:1px solid var(--az-border); }
    .nbn-filters input, .nbn-filters select{ border:1px solid #dcdcdc; padding:.5rem .7rem; border-radius:.35rem; }
    .nbn-filters input{ margin-right:8px; flex:1 1 240px; }
    .nbn-body{ padding:12px 16px; overflow:auto; flex:1; }
    .nbn-grid{ display:grid; grid-template-columns:repeat(auto-fit, minmax(160px,1fr)); }
    .nbn-item{ border:1px solid var(--az-border); padding:8px; cursor:pointer; margin:6px; border-radius:.35rem; }
    .nbn-item.is-sel{ box-shadow:0 0 0 3px #e5e7eb; }
    .nbn-thumb{ height:120px; background:#fafafa; display:flex; align-items:center; justify-content:center; border:1px solid #f0f0f0; position:relative; border-radius:.35rem; }
    .nbn-thumb img, .nbn-thumb video{ max-width:100%; max-height:100%; }
    .nbn-badge{ position:absolute; top:6px; left:6px; background:rgba(0,0,0,.65); color:#fff; border-radius:.35rem; padding:2px 6px; font-size:.75rem; }
    .nbn-title{ margin-top:6px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    .nbn-foot{ display:flex; justify-content:flex-end; padding:12px 16px; border-top:1px solid var(--az-border); }
    .nbn-btn{ border:1px solid #000; background:#000; color:#fff; padding:.5rem .85rem; font-weight:600; cursor:pointer; margin-left:8px; border-radius:.35rem; }
    .nbn-cancel{ background:#444; border-color:#444; }
    .nbn-up{ display:flex; flex-wrap:wrap; padding:12px 16px; border-bottom:1px solid var(--az-border); }
    .nbn-up input[type="file"], .nbn-up input[type="text"], .nbn-up input[type="url"]{ border:1px solid #dcdcdc; padding:.5rem .7rem; margin-right:8px; margin-bottom:8px; border-radius:.35rem; }
    .nbn-btn-active{ background:#16a34a !important; border-color:#16a34a !important; color:#fff !important; }
</style>
        ...
        <div class="row g-3 mt-3">
            ...
            <div class="col-md-6">
                <div class="card h-100">
                    <div class="card-header"><h6 class="card-title mb-0">الصورة</h6></div>
                    <div class="card-body">
                        <div class="mb-3">
                            <div class="input-group">
                                ...
                                >
                                <button type="button" class="btn btn-outline-secondary" id="btnPickShareImage" title="اختيار من nbn">
                                    <i class="fas fa-images"></i>
                                </button>
                                <button type="button" class="btn btn-outline-danger" id="btnClearShareImage" title="حذف">
                                    <i class="fas fa-xmark"></i>
                                </button>
                            </div>
                            {{-- extra hidden fields (optional if you store them) --}}
                            <input type="hidden" id="share_image_id" name="share_image_id" value="{{ old('share_image_id', $content->share_image_id ?? '') }}">
                            <input type="hidden" id="share_image_title" name="share_image_title" value="{{ old('share_image_title', $content->share_image_title ?? '') }}">
                            <input type="hidden" id="share_image_alt" name="share_image_alt" value="{{ old('share_image_alt', $content->share_image_alt ?? '') }}">
                        </div>
                        ...
                        <div class="image-preview-container border rounded p-3 text-center bg-light">
                            <div id="share_image_preview_wrapper" class="d-none">
                                <div class="image-preview-wrapper position-relative d-inline-block">
                                    <img id="share_image_preview" src="" alt="صورة المشاركة" class="img-fluid rounded shadow-sm">
                                    <button type="button"
            ...
            <div class="col-md-6">
                <div class="card h-100">
                    <div class="card-header"><h6 class="card-title mb-0">محتوى المشاركة</h6></div>
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input
                                ...
                                placeholder="أدخل عنوان المشاركة"
                                maxlength="100"
                        ...
                        <div class="mb-0">
                            ...
                            <textarea
                                ...
                                class="form-control"
                                rows="4"
                                placeholder="أدخل وصف المشاركة"
                                maxlength="260"
                                oninput="updatePreview()"
                            >{{ old('share_description', $content->share_description ?? '') }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
...
<div id="nbnMediaPicker" class="nbn-modal" aria-hidden="true" role="dialog" aria-modal="true" aria-labelledby="nbnTitle">
    ...
    <div class="nbn-box" role="document">
            ...
            <div class="nbn-up" style="padding:1.2rem; border-radius:8px; background:#fafbfc; border:1px solid var(--az-border); box-shadow:0 2px 8px rgba(0,0,0,0.04);">
                <div class="nbn-upload-fields" style="display:flex; flex-wrap:wrap; gap:.6rem; width:100%;">
                    <div style="flex:1 1 220px;">
                        <label for="nbnUploadFile" style="display:block; width:100%; cursor:pointer; padding:.6rem .7rem; border:1px solid #dcdcdc; background:#fafafa; color:#333; text-align:center;">
                    ...
                    <div style="flex:1 1 200px;">
                        <input type="text" id="nbnUploadName" placeholder="اسم الملف" style="width:100%; padding:.6rem .7rem; border:1px solid #dcdcdc; background:#fff;">
                    </div>
                    <div style="flex:1 1 200px;">
                        <input type="text" id="nbnUploadAlt" placeholder="النص البديل" style="width:100%; padding:.6rem .7rem; border:1px solid #dcdcdc; background:#fff;">
                    </div>
                </div>
                <div class="nbn-uploader-actions" style="display:flex; gap:.7rem; margin-bottom:.7rem;">
                    <button type="button" class="nbn-btn nbn-btn-secondary" id="nbnUploadToGallery" style="border-radius:6px; font-size:1rem; padding:.7rem 1.2rem;">إدراج في المعرض</button>
                    <button type="button" class="nbn-btn nbn-btn-primary" id="nbnUploadAndClose" style="border-radius:6px; font-size:1rem; padding:.7rem 1.2rem;">إدراج في المقال</button>
                </div>
            </div>
        </section>
        ...
        <section id="nbn-panel-import" class="nbn-panel" role="tabpanel" aria-labelledby="nbn-tab-import" hidden>
            <div class="x-tab-body">
                <div class="x-uploader x-uploader-url" style="padding:1.2rem; border-radius:8px; background:#fafbfc; border:1px solid var(--x-border,#e5e7eb); box-shadow:0 2px 8px rgba(0,0,0,0.04);">
                    <div style="display:flex; flex-wrap:wrap; gap:.7rem; margin-bottom:.7rem;">
                        <input type="text" id="nbnUrl" style="flex:1 1 220px; padding:.7rem 1rem; border:1px solid #dcdcdc; border-radius:6px; background:#fff; font-size:1rem;" placeholder="الرابط">
                        <input type="text" id="nbnUrlName" placeholder="اسم الملف" style="flex:1 1 180px; padding:.7rem 1rem; border:1px solid #dcdcdc; border-radius:6px; background:#fff; font-size:1rem;">
                        <input type="text" id="nbnUrlAlt" placeholder="النص البديل" style="flex:1 1 180px; padding:.7rem 1rem; border:1px solid #dcdcdc; border-radius:6px; background:#fff; font-size:1rem;">
                    </div>
                    <div class="x-uploader-actions" style="display:flex; gap:.7rem; margin-bottom:.7rem;">
                        <button class="x-btn x-btn-secondary nbn-btn" type="button" id="nbnImportToGallery" style="border-radius:6px; font-size:1rem; padding:.7rem 1.2rem;">إدراج في المعرض</button>
                        <button class="x-btn x-btn-primary nbn-btn" type="button" id="nbnImportAndClose" style="border-radius:6px; font-size:1rem; padding:.7rem 1.2rem;">إدراج في المقال</button>
                    </div>
                </div>
            </div>
        </section>
...
(function() {
    ...
    const nbn_FETCH_URL  = azSafeRoute("{{ route('dashboard.media.getAllMediaPaginated') }}", "/dashboard/media/paginated");
    ...
    const state = { open:false, page:1, hasMore:true, isLoading:false, list:[], search:'', type:'all', selected:null, obs:null, cb:null };
    const YT = /^(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/shorts\/)([A-Za-z0-9_-]{6,})/i;
    ...
    function lock(){ document.documentElement.style.overflow='hidden'; document.body.classList.add('modal-open'); }
    function unlock(){ document.documentElement.style.overflow=''; document.body.classList.remove('modal-open'); }

    let prevModal=null, restore=false;
    function hideParent(){
        try{
            ...
            if (shown && window.bootstrap?.Modal){
                ...
                prevModal=shown; restore=true;
                setTimeout(()=>{ document.querySelectorAll('.modal-backdrop').forEach(el=>el.remove()); document.body.classList.remove('modal-open'); document.documentElement.style.overflow=''; },150);
            }
        }catch(e){}
    }
    function showParent(){
        try{
            if(restore && prevModal && window.bootstrap?.Modal){ window.bootstrap.Modal.getOrCreateInstance(prevModal).show(); }
        }catch(e){} finally{
            prevModal=null; restore=false;
    ...
    function open(){ state.open=true; hideParent(); root.setAttribute('aria-hidden','false'); lock(); switchTab('gallery'); resetAndLoad(); document.addEventListener('keydown', onEsc, {passive:true}); }
    function close(){ state.open=false; root.setAttribute('aria-hidden','true'); state.obs?.disconnect(); state.obs=null; state.cb=null; state.selected=null; unlock(); showParent(); document.removeEventListener('keydown', onEsc); }
    function onEsc(e){ if (e.key==='Escape' && root.getAttribute('aria-hidden')==='false'){ e.preventDefault(); close(); } }
    back && back.addEventListener('click', close);
    closes.forEach(b=> b.addEventListener('click', close));

    async function resetAndLoad(){
        state.page=1; state.hasMore=true; state.list=[]; render(); await load();
        state.obs?.disconnect(); state.obs=null;
        const body = panels.gallery?.querySelector('.nbn-body');
        if (!body || !sentinel) return;
        state.obs = new IntersectionObserver(es => es.forEach(e=>{ if(e.isIntersecting) load(); }), { root: body, threshold:1 });
        state.obs.observe(sentinel);
    }
    async function load(){
        if(state.isLoading || !state.hasMore) return;
        state.isLoading=true; loaderEl.hidden=false;
        try{
            ...
            state.page+=1;
        }catch{ state.hasMore=false; } finally{
            state.isLoading=false; loaderEl.hidden=true; render();
        }
    }
    function render(){
        listEl.textContent='';
        const arr = state.type==='all'? state.list : state.list.filter(m => (state.type==='image'? kind(m)==='image' : true));
        if(!arr.length){
        ...
        arr.forEach(m=>{
            ...
            it.addEventListener('click', ()=>{ state.selected = (state.selected && state.selected.id===m.id)? null : m; render(); });
            const th=document.createElement('div'); th.className='nbn-thumb';
            const badge=document.createElement('div'); badge.className='nbn-badge'; badge.textContent=iconText(m); th.appendChild(badge);
            const p=m.path||m.url||'';
            if(isYT(p)){
                const id=youId(p); const img=new Image(); img.loading='lazy'; img.alt=m.name||'YouTube'; img.src=`https://img.youtube.com/vi/${id}/hqdefault.jpg`; th.appendChild(img);
            }else{
                const k=kind(m);
                if(k==='image'){ const img=new Image(); img.loading='lazy'; img.alt=m.alt||m.name||''; img.src=p; th.appendChild(img); }
                else if(k==='video' && /(\.|\/)(mp4|webm|mkv|mov|avi|m4v)(\?|$)/i.test(p)){ const v=document.createElement('video'); v.src=p; v.muted=true; v.preload='metadata'; th.appendChild(v); }
            }
            const tt=document.createElement('div'); tt.className='nbn-title'; tt.textContent=m.name||'';
            it.appendChild(th); it.appendChild(tt); listEl.appendChild(it);
        });
    }
    searchEl && searchEl.addEventListener('input', ()=>{ state.search=searchEl.value; resetAndLoad(); });
    typeEl   && typeEl.addEventListener('change', ()=>{ state.type=typeEl.value; resetAndLoad(); });

    function choose(){
        if(!state.selected){ alert('يرجى اختيار وسيط'); return; }
        const m=state.selected;
        const payload={ id:m.id, url:m.url||m.path||'', title:m.name||'', alt:m.alt||'' };
        if(!payload.url){ alert('الرابط غير متاح'); return; }
        state.cb && state.cb(payload); close();
    }
    ...
    async function doUpload(mode){
        ...
        const form=new FormData(); form.append('media', f);
        ...
        try{
            ...
            const r=await fetch(nbn_UPLOAD_URL, { method:'POST', headers:{ 'X-CSRF-TOKEN':CSRF, Accept:'application/json' }, body:form });
            if(!r.ok) return alert('فشل الرفع');
            await resetAndLoad();
            if(mode==='gallery'){ switchTab('gallery'); }
            else{ const first=state.list[0]||null; if(first && state.cb) state.cb({ id:first.id, url:first.url||first.path, title:first.name||'', alt:first.alt||'' }); close(); }
            ...
            if(upFile) upFile.value=''; if(upName) upName.value=''; if(upAlt) upAlt.value='';
        }
    }
    ...
    async function doImport(mode){
        ...
        const body={ url:u }; if(urlName?.value) body.name=urlName.value; if(urlAlt?.value) body.alt=urlAlt.value;
        try{
            ...
            const r=await fetch(nbn_IMPORT_URL, { method:'POST', headers:{ 'X-CSRF-TOKEN':CSRF, Accept:'application/json', 'Content-Type':'application/json' }, body:JSON.stringify(body) });
            ...
            importAndClose && (importAndClose.disabled=false, importAndClose.textContent='إدراج في المقال');
            if(urlInput) urlInput.value=''; if(urlName) urlName.value=''; if(urlAlt) urlAlt.value='';
        }
    }
    ...
    // Public API
    window.nbnPicker = {
        open(onSelect){ state.cb = (typeof onSelect==='function')? onSelect : null; open(); },
        close(){ close(); }
    };
    ...
    window.setShareImage = function(payload){
        ...
        if(payload.url){
            if(previewImg){ previewImg.src = payload.url; previewImg.alt = payload.alt || payload.title || ''; }
            if(previewName){ previewName.textContent = payload.title || payload.url; }
            previewWrap && previewWrap.classList.remove('d-none');
            placeholder && (placeholder.style.display='none');
            if(prevImg){ prevImg.src = payload.url; prevImg.alt = payload.alt || payload.title || ''; }
    ...
    window.removeShareImage = function(){ setShareImage({ id:'', url:'', title:'', alt:'' }); };
    ...
    function opennbnForSocial(){
        window.nbnPicker.open(function(media){
            if(!media || !media.url){ alert('لا يمكن استخدام هذا الوسيط'); return; }
            setShareImage({ id:media.id||'', url:media.url||'', title:media.title||'', alt:media.alt||'' });
        });
    }
    ...
    document.addEventListener('DOMContentLoaded', function(){
        ...
        form.addEventListener('submit', function(){
            ...
            if(!hidden){
                ...
                hidden.type='hidden'; hidden.name='share_image';
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (initialShare.url && typeof window.setShareImage === 'function') {
            ...
            window.setShareImage({ id:'', url:'', title:'', alt:'' });
        }

            ...
            <div class="card media-card media-card--tall position-relative">
                ...
                <div class="media-select-wrap" style="position:absolute; top:10px; right:10px; z-index:5;">
                    <input type="checkbox" class="media-select-checkbox" value="{{ $media->id }}"
                        @if ($isInUse) disabled title="هذه الوسائط مستخدمة (في محتوى، مقال، أو ملف شخصي)" @endif
                ...
                <div class="card-inner">
                    ...
                    <div class="media-preview-container preview-media" role="button" tabindex="0"
                        style="cursor: pointer;" data-media-id="{{ $media->id }}"
                        data-media-name="{{ $media->name }}" data-media-alt="{{ $media->alt }}"
                        data-media-type="{{ $media->media_type }}" data-path="{{ $media->path }}">
                        ...
                        @elseif($media->media_type === 'video')
                            <div class="media-preview-16-9"
                                ...
                                <div style="text-align: center; color: white;">
                                    <em class="icon ni ni-play-circle"
                                        style="font-size: 4rem; margin-bottom: 0.5rem;"></em>
                        ...
                        @else
                            <div class="media-preview-16-9 document-preview"
                                ...
                                <div class="document-content"
                                    style="text-align: center; color: {{ $isPdf ? '#fff' : '#111827' }};">
                                    @if ($isPdf)
                                    ...
                                    @else
                                        <em class="icon ni ni-file-text"
                                            style="font-size: 3rem; margin-bottom: 0.5rem;"></em>
                                    @endif
                                </div>
                    ...
                    <div class="media-info my-2">
                        <h6 class="media-name"
                            style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-bottom: 0.5rem;">
                            {{ $media->name }}</h6>
                        <div class="media-date text-muted"
                            style="font-size: 0.8rem; display:flex; align-items:center; gap:.35rem;">
                    ...
                    <div class="media-actions">
                        ...
                        <form action="{{ route('dashboard.media.destroy', $media->id) }}" method="POST"
                            ...
                            <button type="button" class="btn btn-dim btn-sm btn-outline-danger delete-btn"
                                @if ($isInUse) disabled title="هذه الوسائط مستخدمة (في محتوى، مقال، أو ملف شخصي) ولا يمكن حذفها" @endif
                                ...
                                data-en-confirm="Yes, delete" data-ar-cancel="إلغاء" data-en-cancel="Cancel">

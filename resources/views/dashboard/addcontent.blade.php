...
@section('content')
    ...
    <script>
        ...
        function filterMultiList(input) {
            ...
            const mydropdown = container.querySelector('.mydropdown');
            const filter = input.value.toLowerCase();
            const optionsList = mydropdown.querySelector('ul');
    ...
    <style>
        /* Apply to the small inline Bootstrap "add" modals on this page */
        ...
        #addWriterLocationModal .btn-close {
            border-radius: 0;
            opacity: .55;
        }
        ...
        #addWriterLocationModal .modal-body {
            padding: 1.25rem;
            background: #fff;
        }
        #addWriterModal .mb-3,
        ...
        /* Social URL fields stacked icon hint (writer modal) */
        #addWriterModal #writer_facebook { background-image: none; }
    ...
    <script>
        (function() {
            // Pick an image from the global mmx media gallery, fetch it as a blob,
            // assign it to the underlying <input type="file"> so the existing form
            // submission keeps working unchanged.
            async function pickFromGallery(fieldEl) {
                ...
                // Move the mmx modal to <body> so it is not trapped inside the
                ...
                window.mmxMediaModalManager.onMediaSelected = async function(media) {
                    // Restore the bridge handler so the media-tab keeps working.

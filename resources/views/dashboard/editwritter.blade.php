...
@section('content')
                ...
                <div class="nk-content">
                    <div class="container">
                        ...
                        <form action="{{ route('dashboard.writer.update', $writer->id) }}" method="POST"
                            ...
                            <!-- صورة الكاتب - Enhanced with Modal -->
                            <div class="form-group">
                                <label class="form-label" for="image" data-en="Image" data-ar="الصورة">الصورة <span
                                        class="text-danger">*</span></label>
                                @if ($writer->image)
                                    {{-- <div class="mb-2">
                                        <img src="{{ $writer->image }}" alt="صورة الكاتب" class="media-preview"
                                            style="max-width: 200px;">
                                    </div> --}}
                                @endif
                                <div class="input-group mb-2">
                                    <input id="writerImageUrl" class="form-control" data-ar="لم يتم الاختيار"
                                        data-en="Not selected" readonly value="{{ old('image', $writer->image) }}" />
                                    <button type="button" class="btn btn-outline-secondary" id="btnPickWriterImage"
                                        data-ar="اختيار الصورة" data-en="Pick image">
                                        <i class="fa fa-images"></i>
                                    </button>
                                    <button type="button" class="btn btn-outline-danger" id="btnClearWriterImage"
                                        title="مسح" data-ar="مسح" data-en="Clear">
                                        <i class="fa fa-xmark"></i>
                                    </button>
                                </div>
                                <input type="hidden" id="image" name="image"
                                    value="{{ old('image', $writer->image) }}">
                                <div id="writerImagePreview" class="mt-2"></div>
                                @error('image')
                                    <span class="text-danger small">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-control-wrap">
                                    <input type="url" name="x" class="form-control" id="x"
                                        value="{{ old('x', $writer->x) }}" placeholder="https://x.com/">
                                </div>
                                @error('x')
                                    <span class="text-danger small">{{ $message }}</span>
                                @enderror
                            ...
                            <!-- زر الإرسال -->
                            <div class="form-group">
                                <button type="submit" class="btn btn-primary" data-en="Edit Writer"
                                    data-ar="تحديث كاتب">
                                    <i class="fa fa-pencil"></i> تحديث كاتب
                                </button>
                                <a href="{{ route('writer.show', $writer->id) }}" target="_blank"
                                    class="btn btn-outline-secondary ml-2" data-ar="عرض" data-en="Preview">عرض</a>
                            </div>
                        </form>
    ...
    <!-- ======================= WRITER MEDIA MODAL ======================= -->
    <div id="writerMediaModal" class="mmxx-modal" aria-hidden="true" role="dialog" aria-modal="true"
        aria-labelledby="writerMediaModalTitle">
        <div class="mmxx-backdrop" data-mmxx-backdrop></div>
        <div class="mmxx-container" role="document">
            <div class="mmxx-header">
                <h5 id="writerMediaModalTitle" data-ar="اختر صورة الكاتب" data-en="Select Writer Image">اختر صورة الكاتب
                </h5>
...
/* === MMX-UI-ENHANCE-V2 === */
#writerMediaModal ,
#writerMediaModal  * { border-radius: 0 !important; }
#writerMediaModal .mmxx-container {
...
#writerMediaModal .mmxx-header {
    padding: 1rem 1.25rem !important;
    background: var(--mmxx-bg, #fff) !important;
    border-bottom: 1px solid rgba(15,23,42,.06) !important;
    flex-shrink: 0;
}
#writerMediaModal .mmxx-header h5 { font-size: 1rem !important; font-weight: 600 !important; letter-spacing: -0.01em; }
#writerMediaModal .mmxx-close {
...
#writerMediaModal .mmxx-close:hover { background: rgba(15,23,42,.05) !important; color: var(--mmxx-text, #1e293b) !important; }

#writerMediaModal .mmxx-tabs {
...
#writerMediaModal .mmxx-tab-btn:hover:not(.mmxx-is-active) {
    background: rgba(15,23,42,.04) !important;
    color: var(--mmxx-text, #1e293b) !important;
}
#writerMediaModal .mmxx-tab-btn.mmxx-is-active {
...
#writerMediaModal .mmxx-tab-panel { display: flex !important; flex-direction: column; flex: 1 1 auto; min-height: 0; overflow: hidden; }
#writerMediaModal .mmxx-tab-panel[hidden] { display: none !important; }

#writerMediaModal .mmxx-filters {
...
#writerMediaModal .mmxx-body::-webkit-scrollbar { width: 6px; }
#writerMediaModal .mmxx-body::-webkit-scrollbar-thumb { background: rgba(15,23,42,.15); border-radius: 0; }
#writerMediaModal .mmxx-body::-webkit-scrollbar-thumb:hover { background: rgba(15,23,42,.25); }

#writerMediaModal .mmxx-grid {
    display: grid !important;
    grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)) !important;
    gap: .75rem !important;
}

#writerMediaModal .mmxx-empty {
    grid-column: 1 / -1;
    text-align: center;
...
#writerMediaModal .mmxx-item:hover {
    border-color: rgba(101,118,255,.4) !important;
    box-shadow: 0 2px 8px rgba(15,23,42,.06) !important;
}
#writerMediaModal .mmxx-item.mmxx-is-selected {
    border-color: var(--mmxx-primary, #6576ff) !important;
    box-shadow: 0 0 0 2px rgba(101,118,255,.25) !important;
}
#writerMediaModal .mmxx-item.mmxx-is-selected::before {
    content: '✓';
    position: absolute;
    top: 8px;
...
#writerMediaModal .mmxx-thumb img {
    width: 100% !important;
...
#writerMediaModal .mmxx-loader {
    text-align: center;
    color: var(--mmxx-muted, #94a3b8);
    padding: 1rem !important;
    font-size: .88rem;
}
#writerMediaModal .mmxx-loader::before {
    content: '';
...
#writerMediaModal .mmxx-pagination {
    display: flex !important;
    flex-wrap: wrap;
    gap: .25rem !important;
    align-items: center;
...
#writerMediaModal .mmxx-pagination button {
...
#writerMediaModal .mmxx-pagination button:hover:not(:disabled) {
    background: rgba(15,23,42,.05) !important;
    color: var(--mmxx-text, #1e293b) !important;
    border-color: transparent !important;
}
#writerMediaModal .mmxx-pagination button.mmxx-page-active {
    background: var(--mmxx-primary, #6576ff) !important;
    border-color: var(--mmxx-primary, #6576ff) !important;
    color: #fff !important;
    box-shadow: none !important;
}
#writerMediaModal .mmxx-pagination button:disabled { opacity: .35; cursor: not-allowed; }
#writerMediaModal .mmxx-pagination .mmxx-page-ellipsis { padding: 0 .25rem; color: var(--mmxx-muted, #94a3b8); }
#writerMediaModal .mmxx-pagination .mmxx-page-info {
    margin-inline-start: auto;
    font-size: .8rem;
    color: var(--mmxx-muted, #94a3b8);
    font-weight: 400;
...
#writerMediaModal .mmxx-btn-primary { background: var(--mmxx-primary, #6576ff) !important; color: #fff !important; border-color: var(--mmxx-primary, #6576ff) !important; }
#writerMediaModal .mmxx-btn-primary:hover:not(:disabled) { background: #5566ee !important; border-color: #5566ee !important; }
#writerMediaModal .mmxx-btn-secondary { background: rgba(15,23,42,.05) !important; color: var(--mmxx-text, #1e293b) !important; }
#writerMediaModal .mmxx-btn-secondary:hover:not(:disabled) { background: rgba(15,23,42,.09) !important; }
#writerMediaModal .mmxx-btn-cancel { background: transparent !important; color: var(--mmxx-muted, #64748b) !important; border-color: rgba(15,23,42,.12) !important; }
#writerMediaModal .mmxx-btn-cancel:hover { background: rgba(15,23,42,.04) !important; color: var(--mmxx-text, #1e293b) !important; }
#writerMediaModal .mmxx-btn-select:not(:disabled) { background: var(--mmxx-primary, #6576ff) !important; color: #fff !important; border-color: var(--mmxx-primary, #6576ff) !important; }
#writerMediaModal .mmxx-btn-select:not(:disabled):hover { background: #5566ee !important; border-color: #5566ee !important; }
#writerMediaModal .mmxx-btn-select:disabled { background: rgba(15,23,42,.05) !important; color: var(--mmxx-muted, #94a3b8) !important; }

#writerMediaModal .mmxx-tab-body {
    padding: 1.25rem !important;
    background: var(--mmxx-bg, #fff) !important;
    overflow: auto;
    flex: 1 1 auto;
...
#writerMediaModal  input[type="text"],
#writerMediaModal  input[type="search"],
#writerMediaModal  input[type="url"],
#writerMediaModal  input[type="email"],
#writerMediaModal  input[type="number"],
#writerMediaModal  input[type="file"],
#writerMediaModal  textarea,
#writerMediaModal  select {
    border-radius: 0 !important;
...
#writerMediaModal  input:focus,
#writerMediaModal  textarea:focus,
#writerMediaModal  select:focus {
    border-color: var(--mmxx-primary, #6576ff) !important;
    box-shadow: 0 0 0 3px rgba(101,118,255,.12) !important;
    outline: none !important;
}
#writerMediaModal  input::placeholder,
#writerMediaModal  textarea::placeholder {
    color: var(--mmxx-muted, #94a3b8) !important;
    opacity: 1;
    font-weight: 400;
}
#writerMediaModal  input[type="file"] { padding: .4rem .55rem !important; cursor: pointer; }
#writerMediaModal  label {
    color: var(--mmxx-text, #334155) !important;
    font-size: .85rem !important;
    font-weight: 500 !important;
    display: inline-block;
...
#writerMediaModal  fieldset {
    border-radius: 0 !important;
    border: 1px solid rgba(15,23,42,.08) !important;
    padding: .7rem 1rem !important;
    background: transparent !important;
}
#writerMediaModal  fieldset legend {
    font-size: .78rem !important;
    color: var(--mmxx-muted, #94a3b8) !important;
    font-weight: 500 !important;
    padding: 0 .4rem !important;
}
#writerMediaModal  .mmxx-radio {
    background: transparent;
...
#writerMediaModal  .mmxx-radio:hover { border-color: rgba(101,118,255,.4); }
#writerMediaModal  .mmxx-radio input[type="radio"] { accent-color: var(--mmxx-primary, #6576ff); margin-inline-end: .25rem; }
#writerMediaModal  .mmxx-uploader,
#writerMediaModal  .mmxx-uploader-url {
    border-radius: 0 !important;
    border: 1px solid rgba(15,23,42,.08) !important;
    background: var(--mmxx-bg, #fff) !important;
    padding: 1rem !important;
...
#writerMediaModal  [id$="-upload-label"] {
    border-radius: 0 !important;
...
#writerMediaModal  [id$="-upload-label"]:hover {
    border-color: var(--mmxx-primary, #6576ff) !important;
...
#writerMediaModal  [id$="-upload-label"][data-file-selected="true"] {
...
#writerMediaModal  [id$="-upload-label"][data-file-selected="true"] i,
#writerMediaModal  [id$="-upload-label"][data-file-selected="true"] em { display: none !important; }
#writerMediaModal  [id$="-upload-label"][data-file-selected="true"]::before {
    content: '✓';
    flex-shrink: 0;
...
#writerMediaModal  [id$="-upload-label"][data-file-selected="true"] span,
#writerMediaModal  [id$="-upload-label"][data-file-selected="true"] [id$="-upload-label-text"] {
    flex: 1 1 auto;
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
...
#writerMediaModal  [id$="-upload-label"][data-file-selected="true"]::after {
    content: 'تغيير';
    flex-shrink: 0;
    font-size: .78rem;
    padding: .25rem .55rem;
...
#writerMediaModal  .mmxx-upload-fields {
    display: grid !important;
...
#writerMediaModal  .mmxx-upload-fields > *:has(> [id$="-upload-label"]),
#writerMediaModal  .mmxx-upload-fields > *:has([id$="-upload-input"]) {
    grid-column: 1 / -1 !important;
}

/* Import-by-URL row layout: URL on its own row, name+alt side-by-side */
#writerMediaModal  .mmxx-uploader-url > div:first-of-type {
    display: grid !important;
    grid-template-columns: 1fr 1fr !important;
    gap: .75rem !important;
    margin-bottom: .85rem !important;
}
#writerMediaModal  .mmxx-uploader-url > div:first-of-type > *:first-child {
    grid-column: 1 / -1 !important;
}
#writerMediaModal  .mmxx-uploader-url > div:first-of-type > * { flex: unset !important; width: 100% !important; min-width: 0 !important; }

/* Radio group: even spacing */
#writerMediaModal  .mmxx-url-type-group > div {
    display: flex !important;
    flex-wrap: wrap;
    gap: .5rem !important;
}
...
/* Action button rows */
#writerMediaModal  .mmxx-uploader-actions {
    display: flex !important;
    gap: .5rem !important;
...
@media (max-width: 640px) {
    ...
    #writerMediaModal  .mmxx-uploader-actions { flex-direction: column; }
    #writerMediaModal  .mmxx-uploader-actions .mmxx-btn { width: 100%; }
}

@media (max-width: 600px) {
    #writerMediaModal .mmxx-pagination .mmxx-page-info { width: 100%; text-align: center; margin-top: .25rem; }
    #writerMediaModal .mmxx-grid { grid-template-columns: repeat(auto-fill, minmax(130px, 1fr)) !important; }
    #writerMediaModal .mmxx-thumb { height: 110px !important; }
}
...
<style>
    ...
    <script>
        (() => {
            ...
            const modal = document.getElementById("writerMediaModal");
            ...
            const imageUrlInput = document.getElementById("writerImageUrl");
            const imagePreview = document.getElementById("writerImagePreview");
            const imageHiddenInput = document.getElementById("image");
            const btnPickImage = document.getElementById("btnPickWriterImage");
            const btnClearImage = document.getElementById("btnClearWriterImage");

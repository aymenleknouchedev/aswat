...
@section('content')
                        ...
                        <div class="card card-bordered">
                            <div class="card-inner">
                                <form action="{{ route('dashboard.user.store') }}" method="POST"
                                    ...
                                    <!-- صورة - Enhanced with Modal -->
                                    <div class="form-group">
                                        <label class="form-label" for="image" data-ar="الصورة"
                                            data-en="Profile Image">الصورة</label>
                                        <div class="input-group mb-2">
                                            <input id="userImageUrl" class="form-control" 
                                                data-ar="لم يتم الاختيار" data-en="Not selected" readonly />
                                            <button type="button" class="btn btn-outline-secondary" id="btnPickUserImage"
                                                data-ar="اختيار الصورة" data-en="Pick image">
                                                <i class="fa fa-images"></i>
                                            </button>
                                            <button type="button" class="btn btn-outline-danger" id="btnClearUserImage"
    ...
    <!-- ======================= USER MEDIA MODAL ======================= -->
    <div id="userMediaModal" class="mmxx-modal" aria-hidden="true" role="dialog" aria-modal="true"
        aria-labelledby="userMediaModalTitle">
        <div class="mmxx-backdrop" data-mmxx-backdrop></div>
        <div class="mmxx-container" role="document">
            <div class="mmxx-header">
                <h5 id="userMediaModalTitle" data-ar="اختر صورة المستخدم" data-en="Select User Image">اختر صورة المستخدم</h5>
...
/* === MMX-UI-ENHANCE-V2 === */
#userMediaModal ,
#userMediaModal  * { border-radius: 0 !important; }
#userMediaModal .mmxx-container {
...
#userMediaModal .mmxx-header {
    padding: 1rem 1.25rem !important;
    background: var(--mmxx-bg, #fff) !important;
    border-bottom: 1px solid rgba(15,23,42,.06) !important;
    flex-shrink: 0;
}
#userMediaModal .mmxx-header h5 { font-size: 1rem !important; font-weight: 600 !important; letter-spacing: -0.01em; }
#userMediaModal .mmxx-close {
...
#userMediaModal .mmxx-close:hover { background: rgba(15,23,42,.05) !important; color: var(--mmxx-text, #1e293b) !important; }

#userMediaModal .mmxx-tabs {
...
#userMediaModal .mmxx-tab-btn:hover:not(.mmxx-is-active) {
    background: rgba(15,23,42,.04) !important;
    color: var(--mmxx-text, #1e293b) !important;
}
#userMediaModal .mmxx-tab-btn.mmxx-is-active {
...
#userMediaModal .mmxx-tab-panel { display: flex !important; flex-direction: column; flex: 1 1 auto; min-height: 0; overflow: hidden; }
#userMediaModal .mmxx-tab-panel[hidden] { display: none !important; }

#userMediaModal .mmxx-filters {
...
#userMediaModal .mmxx-body::-webkit-scrollbar { width: 6px; }
#userMediaModal .mmxx-body::-webkit-scrollbar-thumb { background: rgba(15,23,42,.15); border-radius: 0; }
#userMediaModal .mmxx-body::-webkit-scrollbar-thumb:hover { background: rgba(15,23,42,.25); }

#userMediaModal .mmxx-grid {
    display: grid !important;
    grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)) !important;
    gap: .75rem !important;
}

#userMediaModal .mmxx-empty {
    grid-column: 1 / -1;
    text-align: center;
...
#userMediaModal .mmxx-item:hover {
    border-color: rgba(101,118,255,.4) !important;
    box-shadow: 0 2px 8px rgba(15,23,42,.06) !important;
}
#userMediaModal .mmxx-item.mmxx-is-selected {
    border-color: var(--mmxx-primary, #6576ff) !important;
    box-shadow: 0 0 0 2px rgba(101,118,255,.25) !important;
}
#userMediaModal .mmxx-item.mmxx-is-selected::before {
    content: '✓';
    position: absolute;
    top: 8px;
...
#userMediaModal .mmxx-loader {
    text-align: center;
    color: var(--mmxx-muted, #94a3b8);
    padding: 1rem !important;
    font-size: .88rem;
}
#userMediaModal .mmxx-loader::before {
    content: '';
...
#userMediaModal .mmxx-pagination {
    display: flex !important;
    flex-wrap: wrap;
    gap: .25rem !important;
    align-items: center;
...
#userMediaModal .mmxx-pagination button {
...
#userMediaModal .mmxx-pagination button:hover:not(:disabled) {
    background: rgba(15,23,42,.05) !important;
    color: var(--mmxx-text, #1e293b) !important;
    border-color: transparent !important;
}
#userMediaModal .mmxx-pagination button.mmxx-page-active {
    background: var(--mmxx-primary, #6576ff) !important;
    border-color: var(--mmxx-primary, #6576ff) !important;
    color: #fff !important;
    box-shadow: none !important;
}
#userMediaModal .mmxx-pagination button:disabled { opacity: .35; cursor: not-allowed; }
#userMediaModal .mmxx-pagination .mmxx-page-ellipsis { padding: 0 .25rem; color: var(--mmxx-muted, #94a3b8); }
#userMediaModal .mmxx-pagination .mmxx-page-info {
    margin-inline-start: auto;
    font-size: .8rem;
    color: var(--mmxx-muted, #94a3b8);
    font-weight: 400;
...
#userMediaModal .mmxx-btn-primary { background: var(--mmxx-primary, #6576ff) !important; color: #fff !important; border-color: var(--mmxx-primary, #6576ff) !important; }
#userMediaModal .mmxx-btn-primary:hover:not(:disabled) { background: #5566ee !important; border-color: #5566ee !important; }
#userMediaModal .mmxx-btn-secondary { background: rgba(15,23,42,.05) !important; color: var(--mmxx-text, #1e293b) !important; }
#userMediaModal .mmxx-btn-secondary:hover:not(:disabled) { background: rgba(15,23,42,.09) !important; }
#userMediaModal .mmxx-btn-cancel { background: transparent !important; color: var(--mmxx-muted, #64748b) !important; border-color: rgba(15,23,42,.12) !important; }
#userMediaModal .mmxx-btn-cancel:hover { background: rgba(15,23,42,.04) !important; color: var(--mmxx-text, #1e293b) !important; }
#userMediaModal .mmxx-btn-select:not(:disabled) { background: var(--mmxx-primary, #6576ff) !important; color: #fff !important; border-color: var(--mmxx-primary, #6576ff) !important; }
#userMediaModal .mmxx-btn-select:not(:disabled):hover { background: #5566ee !important; border-color: #5566ee !important; }
#userMediaModal .mmxx-btn-select:disabled { background: rgba(15,23,42,.05) !important; color: var(--mmxx-muted, #94a3b8) !important; }

#userMediaModal .mmxx-tab-body {
    padding: 1.25rem !important;
    background: var(--mmxx-bg, #fff) !important;
    overflow: auto;
    flex: 1 1 auto;
...
#userMediaModal  input[type="text"],
#userMediaModal  input[type="search"],
#userMediaModal  input[type="url"],
#userMediaModal  input[type="email"],
#userMediaModal  input[type="number"],
#userMediaModal  input[type="file"],
#userMediaModal  textarea,
#userMediaModal  select {
    border-radius: 0 !important;
...
#userMediaModal  input:focus,
#userMediaModal  textarea:focus,
#userMediaModal  select:focus {
    border-color: var(--mmxx-primary, #6576ff) !important;
    box-shadow: 0 0 0 3px rgba(101,118,255,.12) !important;
    outline: none !important;
}
#userMediaModal  input::placeholder,
#userMediaModal  textarea::placeholder {
    color: var(--mmxx-muted, #94a3b8) !important;
    opacity: 1;
    font-weight: 400;
}
#userMediaModal  input[type="file"] { padding: .4rem .55rem !important; cursor: pointer; }
#userMediaModal  label {
    color: var(--mmxx-text, #334155) !important;
    font-size: .85rem !important;
    font-weight: 500 !important;
    display: inline-block;
...
#userMediaModal  fieldset {
    border-radius: 0 !important;
    border: 1px solid rgba(15,23,42,.08) !important;
    padding: .7rem 1rem !important;
    background: transparent !important;
}
#userMediaModal  fieldset legend {
    font-size: .78rem !important;
    color: var(--mmxx-muted, #94a3b8) !important;
    font-weight: 500 !important;
    padding: 0 .4rem !important;
}
#userMediaModal  .mmxx-radio {
    background: transparent;
...
#userMediaModal  .mmxx-radio:hover { border-color: rgba(101,118,255,.4); }
#userMediaModal  .mmxx-radio input[type="radio"] { accent-color: var(--mmxx-primary, #6576ff); margin-inline-end: .25rem; }
#userMediaModal  .mmxx-uploader,
#userMediaModal  .mmxx-uploader-url {
    border-radius: 0 !important;
    border: 1px solid rgba(15,23,42,.08) !important;
    background: var(--mmxx-bg, #fff) !important;
    padding: 1rem !important;
...
#userMediaModal  [id$="-upload-label"] {
    border-radius: 0 !important;
...
#userMediaModal  [id$="-upload-label"]:hover {
    border-color: var(--mmxx-primary, #6576ff) !important;
...
#userMediaModal  [id$="-upload-label"][data-file-selected="true"] {
...
#userMediaModal  [id$="-upload-label"][data-file-selected="true"] i,
#userMediaModal  [id$="-upload-label"][data-file-selected="true"] em { display: none !important; }
#userMediaModal  [id$="-upload-label"][data-file-selected="true"]::before {
    content: '✓';
    flex-shrink: 0;
...
#userMediaModal  [id$="-upload-label"][data-file-selected="true"] span,
#userMediaModal  [id$="-upload-label"][data-file-selected="true"] [id$="-upload-label-text"] {
    flex: 1 1 auto;
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
...
#userMediaModal  [id$="-upload-label"][data-file-selected="true"]::after {
    content: 'تغيير';
    flex-shrink: 0;
    font-size: .78rem;
    padding: .25rem .55rem;
...
#userMediaModal  .mmxx-upload-fields {
    display: grid !important;
...
#userMediaModal  .mmxx-upload-fields > *:has(> [id$="-upload-label"]),
#userMediaModal  .mmxx-upload-fields > *:has([id$="-upload-input"]) {
    grid-column: 1 / -1 !important;
}

/* Import-by-URL row layout: URL on its own row, name+alt side-by-side */
#userMediaModal  .mmxx-uploader-url > div:first-of-type {
    display: grid !important;
    grid-template-columns: 1fr 1fr !important;
    gap: .75rem !important;
    margin-bottom: .85rem !important;
}
#userMediaModal  .mmxx-uploader-url > div:first-of-type > *:first-child {
    grid-column: 1 / -1 !important;
}
#userMediaModal  .mmxx-uploader-url > div:first-of-type > * { flex: unset !important; width: 100% !important; min-width: 0 !important; }

/* Radio group: even spacing */
#userMediaModal  .mmxx-url-type-group > div {
    display: flex !important;
    flex-wrap: wrap;
    gap: .5rem !important;
}
...
/* Action button rows */
#userMediaModal  .mmxx-uploader-actions {
    display: flex !important;
    gap: .5rem !important;
...
@media (max-width: 640px) {
    ...
    #userMediaModal  .mmxx-uploader-actions { flex-direction: column; }
    #userMediaModal  .mmxx-uploader-actions .mmxx-btn { width: 100%; }
}

@media (max-width: 600px) {
    #userMediaModal .mmxx-pagination .mmxx-page-info { width: 100%; text-align: center; margin-top: .25rem; }
    #userMediaModal .mmxx-grid { grid-template-columns: repeat(auto-fill, minmax(130px, 1fr)) !important; }
    #userMediaModal .mmxx-thumb { height: 110px !important; }
}
...
<style>
    ...
    <script>
        (() => {
            ...
            const modal = document.getElementById("userMediaModal");
            ...
            const imageUrlInput = document.getElementById("userImageUrl");
            const imagePreview = document.getElementById("userImagePreview");
            const imageHiddenInput = document.getElementById("image");
            const btnPickImage = document.getElementById("btnPickUserImage");
            const btnClearImage = document.getElementById("btnClearUserImage");
